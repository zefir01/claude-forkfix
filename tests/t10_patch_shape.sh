...
# removed line 1: the fork/non-fork system-prompt selection in $W
grep -q '^-at=d?.systemPrompt?d.systemPrompt:lf(await Cgv(e,r,ie,ft)),$' "$P" \
  || { echo "  FAIL unexpected removed line ($W system prompt)"; rc=1; }
grep -q '^+at=d?.systemPrompt?ffxForkSystemPrompt(d.systemPrompt,e,t,ee):lf(await Cgv(e,r,ie,ft)),$' "$P" \
  || { echo "  FAIL unexpected replacement line ($W system prompt)"; rc=1; }
# removed line 2: the shell-shim exec path (see tests/t12 for what it fixes)
grep -q '^-if(c\[TJs\]=process.execPath,l)c.TMUX=l;$' "$P" \
  || { echo "  FAIL unexpected removed line (shim exec path)"; rc=1; }
grep -q '^+if(c\[TJs\]=process.env.CLAUDE_CODE_EXECPATH||process.execPath,l)c.TMUX=l;$' "$P" \
...
if not m or m.group(1).strip() != 'if (t?.agentType !== rxe) return e;':
    print("  FAIL non-fork guard is not the first statement"); sys.exit(1)
if 'rxe' not in added:
    print("  FAIL fork type constant not used"); sys.exit(1)
print("  non-fork guard present: `if (t?.agentType !== rxe) return e;` (identical value returned)")
PY
rc=$(( rc + $? ))
exit $rc
